...
社内在庫管理システム(zaikoSystem)。現行は Laravel 10 + Breeze(Inertia/React) + MySQL 構成。中核機能は「在庫一覧の閲覧・検索・行編集(入出庫)・一括入庫登録・CSV ダウンロード」の 1 画面完結型アプリである。
...
| DB | MySQL(業務テーブルは既存を継続利用) | 同一 MySQL(業務 4 テーブルは無変更、認証系テーブルは Better Auth 生成スキーマに置換) |
...
- **認証**: 必須。
- **入力**(JSON): `items` = `{ productName, modelNumber, location, inventoryItem, remarks }` の配列。フロントからは `inventoryItem` が**文字列**で届く(現行 MySQL は暗黙変換で受けている)。API 自体は複数要素の配列を処理できるが、現行 UI からは常に 1 要素の配列が届く(§2.3)。
...
**前提**: 新旧システムを**同一 MySQL データベース(業務 4 テーブルを共有)**に接続して並走させ、画面ごと・操作ごとに結果を突き合わせる。認証系テーブルは別系統(旧: users / 新: Better Auth スキーマ)のため、両系に同等のテストユーザーを用意しておく。更新系操作は「旧で実行→両画面で確認」「新で実行→両画面で確認」の双方向で行う。
