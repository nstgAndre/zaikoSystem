...
/**
 * 在庫管理一覧画面(現行 Pages/Index.tsx = InventoryDashboard の忠実移植)。
 * 列数変化の非対称(ヘッダは「いずれかの行が編集中」で 8 列、行は該当行のみ 8 列)、
 * エラーメッセージ非表示、1 ページ 3 件固定などの現行仕様を意図的に維持
 * (migration-spec.md §2.2)。入庫記録モーダルと CSV ダウンロードの実体は PR5。
 */
export function IndexPage() {
  const { data: session } = authClient.useSession();

  const { showRegisterModal, setShowRegisterModal, searchValue, setSearchValue } =
    useInventoryItemState();

  const { items = [], loading, checkBox, setCheckBox, fetchData } = useFetchItemsData();

  const { modalShow, selectedRemark, openModal, closeModal } = useModalRemark();

  const { handleDownloadCsv } = useDownloadCsv(checkBox, setCheckBox);

  const { currentPage, pageCount, itemsDisplayed, handlePageClick } = usePagenateSearchFilter({
    items,
    searchValue,
  });

  const { handleEditButtonClick, btnEditChangeColors } = useEditUpdate(fetchData);
  ...
  return (
    <AuthenticatedLayout
      user={session.user}
      header={
        <h2 className="font-semibold text-xl pb-1 text-white leading-tight text-left">
          在庫管理一覧
        </h2>
      }
    >
      <div className="mx-auto sm:px-6 lg:px-8">
            ...
            <div>
              <div className="flex justify-end space-x-4">
                <DangerButton
                  ...
                  className="text-white border-2 !bg-deepblue !border-lightblue rounded-md mb-2 !focus:ring-blue-500"
                >
                  入庫記録
                </DangerButton>
                <DangerButton
              ...
              <table className="w-full border-4 border-lightblue bg-deepblue">
                  ...
                  <tr
                    className={`text-white ${
                      Object.values(btnEditChangeColors).includes('lightred')
                        ? 'grid grid-cols-8'
                        : 'grid grid-cols-7'
                    } text-white border-b-2 border-lightblue mr-2 ml-2`}
                  >
                ...
                <tbody>
                  {itemsDisplayed.map((item) => (
                    <tr
                      key={item.id}
                      className={`mt-4 mr-2 ml-2 mb-2 text-white border-2 border-lightblue rounded-md ${
                    ...
                    >
                      ...
                      <td className="py-3 px-4 text-center">
                        {btnEditChangeColors[item.id] === 'lightred' ? (
                          ...
                          <DangerButton
                            onClick={() => openModal(item.remarks)}
                            className="text-white !bg-gold"
                          >
                            詳細
                          </DangerButton>
                        )}
                      ...
                      <td className="py-3 px-4 text-center">
                        <div className="flex items-center justify-center">
                          ...
                          >
                            <FontAwesomeIcon
                              icon={
                                btnEditChangeColors[item.id] === 'lightred' ? faArrowsRotate : faPen
                              }
                            />
                          </DangerButton>
                        </div>
        ...
        <div className="flex justify-center mt-4">
          ...
          {Array.from({ length: pageCount }, (_, index) => (
            <button
              type="button"
              // biome-ignore lint/suspicious/noArrayIndexKey: ページ番号そのものが識別子
              key={index}
              onClick={() => handlePageClick(index)}
              className={`px-3 py-1 mx-1 ${
                index === currentPage ? 'bg-blue-700' : 'bg-blue-500'
              } text-white rounded`}
            >
              {index + 1}
            </button>
